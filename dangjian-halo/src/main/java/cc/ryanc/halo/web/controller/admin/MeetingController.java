...
@Slf4j
@Controller
@RequestMapping(value = "/admin/meetings")
public class MeetingController extends BaseController {
     ...
     * @param page  当前页码
     * @param size  每页显示的条数
     * @return 模板路径admin/admin_meeting
     */
    @GetMapping
    public String posts(Model model,
        ...
        Page<Post> posts = postService.findPostByStatus(status, PostTypeEnum.POST_TYPE_MEETING.getDesc(), pageable);
        model.addAttribute("posts", posts);
        model.addAttribute("publishCount", postService.getCountByStatus(PostStatusEnum.PUBLISHED.getCode(), PostTypeEnum.POST_TYPE_MEETING.getDesc()));
        model.addAttribute("draftCount", postService.getCountByStatus(PostStatusEnum.DRAFT.getCode(), PostTypeEnum.POST_TYPE_MEETING.getDesc()));
        model.addAttribute("trashCount", postService.getCountByStatus(PostStatusEnum.RECYCLE.getCode(), PostTypeEnum.POST_TYPE_MEETING.getDesc()));
        model.addAttribute("status", status);
        return "admin/admin_meeting";
    }
     ...
     * @param postId 文章编号
     * @param model  model
     * @return 模板路径/themes/{theme}/meeting
     */
    ...
    public String writePost() {
        return "admin/admin_meeting_new";
    }
    ...
    public String editPost(@RequestParam("postId") Long postId, Model model) {
        ...
        return "admin/admin_meeting_edit";
    }
    ...
    public JsonResult save(@ModelAttribute Post post,
        ...
        try {
            ...
            post.setPostType(PostTypeEnum.POST_TYPE_MEETING.getDesc());
            ...
            postService.save(post, cateList);
            logsService.save(LogsRecord.PUSH_MEETING, post.getPostTitle(), request);
    ...
    public JsonResult update(@ModelAttribute Post post,
        ...
        post = postService.save(post, cateList);
        if (null != post) {
    ...
    public String moveToPublish(@RequestParam("postId") Long postId,
        ...
        return "redirect:/admin/meetings?status=" + status;
    }
    ...
    public String removePost(@RequestParam("postId") Long postId, @RequestParam("postType") String postType) {
        try {
            ...
            logsService.save(LogsRecord.REMOVE_MEETING, post.get().getPostTitle(), request);
        } catch (Exception e) {
            log.error("Delete article failed: {}", e.getMessage());
        }
        if (StrUtil.equals(PostTypeEnum.POST_TYPE_MEETING.getDesc(), postType)) {
            return "redirect:/admin/meetings?status=2";
        }
    ...
    public JsonResult checkUrlExists(@RequestParam("postUrl") String postUrl) {
        ...
        Post post = postService.findByPostUrl(postUrl, PostTypeEnum.POST_TYPE_MEETING.getDesc());
        if (null != post) {
    ...
    public JsonResult pushAllToBaidu(@RequestParam("baiduToken") String baiduToken) {
        ...
        List<Post> posts = postService.findAll(PostTypeEnum.POST_TYPE_MEETING.getDesc());
        StringBuilder urls = new StringBuilder();
        for (Post post : posts) {
            urls.append(blogUrl);
            urls.append("/meetings/");
