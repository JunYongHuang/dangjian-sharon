...
public interface PostRepository extends JpaRepository<Post, Long> {
     ...
     * @return List
     */
    @Query(value = "select year(post_date) as year,count(*) as count from halo_post where post_status=0 and post_type=:postType group by year(post_date) order by year desc", nativeQuery = true)
    List<Object[]> findPostGroupByYear(@Param("postType") String postType);
     ...
     * @return List
     */
    @Query(value = "select *,year(post_date) as year from halo_post where post_status=0 and post_type=:postType and year(post_date)=:year order by post_date desc", nativeQuery = true)
    List<Post> findPostByYear(@Param("year") String year, @Param("postType") String postType);
     ...
     * @return Page
     */
    Page<Post> findPostByCategoriesAndPostStatusAndPostType(Category category, Integer status, String postType, Pageable pageable);
    ...
    Page<Post> findPostBySpecialTypeAndPostStatus(SpecialType specialType, Integer code, Pageable pageable);

    @Query("select b from Post b, in(b.categories) cc where  cc.roleId = :roleId  and b.postStatus = :status and b.postType = :postType")
    Page<Post> findByRoleIdAndStatus(Integer roleId, int status, String postType, Pageable pageable);

    Page<Post> findByWhite(White white, Pageable pageable);

    List<Post> findAllByPostTypeAndPostStatus(String postType, Integer postStatus);
}
